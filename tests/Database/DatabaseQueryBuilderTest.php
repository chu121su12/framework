...
class DatabaseQueryBuilderTest extends TestCase
{
    ...
    public function testWhereAll()
    {
        ...
        $builder->select('*')->from('users')->whereAll([
            function (Builder $query) { return $query->where('last_name', 'like', '%Otwell%'); },
            function (Builder $query) { return $query->where('email', 'like', '%Otwell%'); },
        ]);
...
SQL;
    ...
    public function testSqlServerLimitsAndOffsets()
    {
        ...
        $this->assertContains($builder->toSql(), [
            ...
            'select * from [users] order by (SELECT 0)',
        ]);
        ...
        $this->assertContains($builder->toSql(), [
            ...
            'select * from [users] where [email] = ? order by (select top 1 [created_at] from [logins] where [users].[name] = ? and [user_id] = [users].[id]) asc',
        ]);
