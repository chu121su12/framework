...
PATTERN;
    ...
    public function testLimit()
    {
        ...
        $this->assertSame('Laravel is a...', Str::limit('Laravel is a free, open source PHP web application framework.', 15, /*$end = */'...', /*preserveWords: */true));

        $string = 'The PHP framework for web artisans.';
        $this->assertSame('The PHP...', Str::limit($string, 7));
        $this->assertSame('The PHP...', Str::limit($string, 10, /*$end = */'...', /*preserveWords: */true));
        $this->assertSame('The PHP', Str::limit($string, 7, ''));
        $this->assertSame('The PHP', Str::limit($string, 10, '', true));
        $this->assertSame('The PHP framework for web artisans.', Str::limit($string, 100));
        $this->assertSame('The PHP framework for web artisans.', Str::limit($string, 100, /*$end = */'...', /*preserveWords: */true));
        $this->assertSame('The PHP framework...', Str::limit($string, 20, /*$end = */'...', /*preserveWords: */true));

        $nonAsciiString = '这是一段中文';
        $this->assertSame('这是一...', Str::limit($nonAsciiString, 6));
        $this->assertSame('这是一...', Str::limit($nonAsciiString, 6, /*$end = */'...', /*preserveWords: */true));
        $this->assertSame('这是一', Str::limit($nonAsciiString, 6, ''));
    ...
    public function testWhetherTheNumberOfGeneratedCharactersIsEquallyDistributed()
    {
        ...
        for ($i = 0; $i < 620000; $i++) {
            $random = Str::random(1);
            $results[$random] = (isset($results[$random]) ? $results[$random] : 0) + 1;
        }
    ...
    public function testItCreatesUlidsNormallyAfterFailureWithinFreezeMethod()
    {
        try {
            Str::freezeUlids(function () {
                Str::createUlidsUsing(function () { return Str::of('1234'); });
    ...
    public function testItCanSpecifyAFallbackForAUlidSequence()
    {
        Str::createUlidsUsingSequence(
            [Str::ulid(), Str::ulid()],
            function () { throw new Exception('Out of Ulids'); }
        );
        Str::ulid();
        Str::ulid();
