...
class PackageManifest extends IlluminatePackageManifest
{
    ...
    protected function getManifest()
    {
        ...
        if (! \is_null($this->testbench) && method_exists($this->testbench, 'ignorePackageDiscoveriesFrom')) {
            $ignorePackageDiscoveriesFrom = $this->testbench->ignorePackageDiscoveriesFrom();
            if (isset($ignorePackageDiscoveriesFrom)) {
                $ignore = $ignorePackageDiscoveriesFrom;
            }
        }
        ...
        return Collection::make(parent::getManifest())
            ->reject(function ($configuration, $package) use ($ignore, $ignoreAll) {
                ...
                foreach (isset($configuration['providers']) ? $configuration['providers'] : [] as $provider) {
    ...
    protected function providersFromRoot()
    {
        ...
        $package = transform(file_get_contents(TESTBENCH_WORKING_PATH.'/composer.json'), function ($json) {
            return backport_json_decode($json, true);
        });

        return [
            $this->format($package['name']) => isset($package['extra']) && isset($package['extra']['laravel']) ? $package['extra']['laravel'] : [],
        ];
    }
