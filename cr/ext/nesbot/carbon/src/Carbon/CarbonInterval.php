...
class CarbonInterval extends DateInterval implements CarbonConverterInterface
{
    ...
    {
        $floatSettersEnabled = backport_type_check('bool', $floatSettersEnabled);
    ...
    public function __construct($years = 1, $months = null, $weeks = null, $days = null, $hours = null, $minutes = null, $seconds = null, $microseconds = null)
    {
        ...
        if (isset($exception)) {
            ...
            } catch (\ErrorException $secondException) {
            } catch (Throwable $secondException) {
            }

            if (isset($secondException)) {
    ...
    private static function castIntervalToClass(DateInterval $interval, /*string */$className, array $skip = [])
    {
        ...
        if (\version_compare(\PHP_VERSION, '7.1.0', '>=') || property_exists($interval, 'f')) {

        $microseconds = $interval->f;
        $instance = new $className(static::getDateIntervalSpec($interval, false, $skip));

        if ($microseconds && property_exists($instance, 'f')) {
    ...
    {
        $skipCopy = backport_type_check('bool', $skipCopy);
    ...
    protected function getForHumansParameters($syntax = null, $short = false, $parts = -1, $options = null)
    {
        ...
        if ($join === false) {
            ...
            $translationMessage = $this->getTranslationMessage('list.1');
            $join = [
                $default,
                isset($translationMessage) ? $translationMessage : $default,
            ];
        }
    ...
    {
        $microseconds = backport_type_check('bool', $microseconds);
        ...
        if ($microseconds && property_exists($interval, 'f') && $interval->f > 0) {
    ...
    private function doCascade(/*bool */$deep)
    {
        ...
        foreach (self::getFlipCascadeFactors() as $source => list($target, $factor)) {
            ...
            if ($decimalPart !== 0.0) {
                ...
                foreach ($previous as list($subUnit, $subFactor)) {
