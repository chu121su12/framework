...
class CarbonPeriod implements Iterator, Countable, JsonSerializable
{
    ...
    public function setStartDate($date, $inclusive = null)
    {
        ...
        if (!$this->isInfiniteDate($date) && !($date = $callableArray($date))) {
    ...
    {
        $callableArray = [$this->dateClass, 'make'];
        if ($date !== null && !$this->isInfiniteDate($date) && !$date = $callableArray($date)) {
    ...
    public function rewind()
    {
        ...
        $this->key = 0;
        $this->current = $callableArray($this->startDate);
    ...
    public function toString()
    {
        ...
        $callableArray = [$this->dateClass, 'getTranslator'];
        $translator = $callableArray();
    ...
    public function first()
    {
        ...
        return isset($asArray[0]) ? $asArray[0] : null;
    }
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'microsecond':
                ...
                $callableArray = [$this->isDefaultInterval ? new CarbonInterval('PT0S') : $this->dateInterval, $method];
    ...
    public function calculateEnd(/*string */$rounding = null)
    {
        ...
        $recurrencesEnd = $this->getEndFromRecurrences();
    ...
    public function round($precision = null, $function = 'round')
    {
        return $this->roundWith(
            isset($precision) ? $precision : $this->getDateInterval()->setLocalTranslator(TranslatorImmutable::get('en'))->forHumans(),
            $function
        );
    }
    ...
    {
        $callableArray = [$this->dateClass, 'hasMacro'];

        return \is_string($callable) && str_starts_with($callable, 'is') &&
            (method_exists($this->dateClass, $callable) || $callableArray($callable));
    }
    ...
    protected function validateCurrentDate()
    {
        ...
        return isset($this->validationResult) ? $this->validationResult : ($this->validationResult = $this->checkFilters());
    }
    ...
    {
        foreach ($defaults as list($index, $name, $value)) {
