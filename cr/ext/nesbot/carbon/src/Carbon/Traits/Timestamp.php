...
trait Timestamp
{
    ...
    {
        list($integer, $decimal) = self::getIntegerAndDecimalParts($timestamp);
    ...
    {
        list($milliseconds, $microseconds) = self::getIntegerAndDecimalParts($timestamp, 3);
    ...
    private static function getIntegerAndDecimalParts($numbers, $decimals = 6)
    {
        ...
        foreach (preg_split('`[^0-9.]+`', $numbers) as $chunk) {
            list($integerPart, $decimalPart) = explode('.', "$chunk.");
