...
trait Difference
{
    ...
    public function diffInMonths($date = null, $absolute = true)
    {
        ...
        list($yearStart, $monthStart, $dayStart) = explode('-', $this->format('Y-m-dHisu'));
        list($yearEnd, $monthEnd, $dayEnd) = explode('-', $date->format('Y-m-dHisu'));
    ...
    public function diffForHumans($other = null, $syntax = null, $short = false, $parts = 1, $options = null)
    {
        ...
        $intSyntax = (int) (isset($intSyntax) ? $intSyntax : static::DIFF_RELATIVE_AUTO);
        $intSyntax = $intSyntax === static::DIFF_RELATIVE_AUTO && $other === null ? static::DIFF_RELATIVE_TO_NOW : $intSyntax;

        $parts = min(7, max(1, (int) $parts));
        $skip = \is_array($syntax) ? (isset($syntax['skip']) ? $syntax['skip'] : []) : [];
