...
<?php $channelIsDefined = isset($logs[0]['channel']); ?>
    ...
    foreach ($logs as $log) {
        if ($log['priority'] >= 400) {
            ...
            $severity = 0;
            if (($exception = (isset($log['context']) && isset($log['context']['exception']) ? $log['context']['exception'] : null)) instanceof \ErrorException || $exception instanceof \Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
        ...
        <tr class="status-<?= $status; ?>" data-filter-level="<?= strtolower($this->escape($log['priorityName'])); ?>"<?php if ($channelIsDefined) { ?> data-filter-channel="<?= $this->escape($log['channel']); ?>"<?php } ?>>
            ...
            <td>
                ...
                <?php if ($log['context']) { ?>
                <pre class="text-muted prewrap m-t-5"><?= $this->escape(backport_json_encode($log['context'], \JSON_PRETTY_PRINT | \JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES)); ?></pre>
                <?php } ?>
            </td>
        </tr>
    <?php
