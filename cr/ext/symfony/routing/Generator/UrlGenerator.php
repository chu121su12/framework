...
class UrlGenerator implements UrlGeneratorInterface, ConfigurableRequirementsInterface
{
    ...
    {
        $enabled = backport_type_check('?bool', $enabled);
        ...
        foreach ($tokens as $token) {
            if ('variable' === $token[0]) {
                ...
                if (!$optional || $important || !\array_key_exists($varName, $defaults) || (null !== $mergedParams[$varName] && (string) $mergedParams[$varName] !== (string) $defaults[$varName])) {
                    ...
                    if (null !== $this->strictRequirements && !preg_match('#^'.preg_replace('/\(\?(?:=|<=|!|<!)((?:[^()\\\\]+|\\\\.|\((?1)\))*)\)/', '', $token[2]).'$#i'.(empty($token[4]) ? '' : 'u'), isset($mergedParams[$token[3]]) ? $mergedParams[$token[3]] : '')) {
    ...
    {
        $targetPath = backport_type_check('string', $targetPath);
