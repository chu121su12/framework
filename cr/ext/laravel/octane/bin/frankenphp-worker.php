...
if ((! (isset($_SERVER['FRANKENPHP_WORKER']) ? $_SERVER['FRANKENPHP_WORKER'] : false)) || ! function_exists('frankenphp_handle_request')) {
...
$worker = null;
$requestCount = 0;
$maxRequests = isset($_ENV['MAX_REQUESTS']) ? $_ENV['MAX_REQUESTS'] : $_SERVER['MAX_REQUESTS'];
$requestMaxExecutionTime = isset($_ENV['REQUEST_MAX_EXECUTION_TIME']) ? $_ENV['REQUEST_MAX_EXECUTION_TIME'] : (isset($_SERVER['REQUEST_MAX_EXECUTION_TIME']) ? $_SERVER['REQUEST_MAX_EXECUTION_TIME'] : null);
...
try {
    $handleRequest = static function () use (&$worker, $basePath, $frankenPhpClient) {
        try {
            ...
            list($request, $context) = $frankenPhpClient->marshalRequest(new RequestContext());
