...
class StartFrankenPhpCommand extends Command implements SignalableCommandInterface
{
    ...
    public function handle(ServerProcessInspector $inspector, ServerStateFile $serverStateFile)
    {
        ...
        $process = tap(new Process([
            ...
            'CADDY_GLOBAL_OPTIONS' => sprintf(
                '%s %s',
                (isset($debug['level']) ? $debug['level'] : '') === 'debug' ? 'debug' : '',
    ...
    protected function buildMercureConfig()
    {
        ...
        if (! $mercure = (isset($octaneConfig['mercure']) ? $octaneConfig['mercure'] : false)) {
            return '';
        }
    ...
    {
        list($_, $errorOutput) = $this->getServerOutput($server);
        ...
        if ($this->option('log-level') !== null) {
            return $errorOutput->each(function ($output) { return $this->raw($output); });
        }
        ...
        $errorOutput->each(function ($output) {
            ...
            if (is_array($stream = backport_json_decode($message, true))) {
