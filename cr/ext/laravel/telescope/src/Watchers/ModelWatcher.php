...
class ModelWatcher extends Watcher
{
    ...
    {
        $app['events']->listen(isset($this->options['events']) ? $this->options['events'] : 'eloquent.*', [$this, 'recordAction']);
    ...
    public function recordAction($event, $data)
    {
        ...
        $modelClass = FormatModel::given(isset($data['model']) ? $data['model'] : $data[0]);

        $changes = with(isset($data['model']) ? $data['model'] : $data[0])->getChanges();
    ...
    {
        if (! (isset($this->options['hydrations']) ? $this->options['hydrations'] : false)
            || ! $this->shouldRecordHydration($modelClass = get_class(isset($data['model']) ? $data['model'] : $data[0]))) {
            return;
        }
    ...
    {
        return collect(isset($this->options['ignore']) ? $this->options['ignore'] : [EntryModel::class])
