...
class BatchWatcher extends Watcher
{
    ...
    public function recordBatch(BatchDispatched $event)
    {
        ...
        $content = array_merge($event->batch->toArray(), [
            'queue' => isset($event->batch->options['queue']) ? $event->batch->options['queue'] : 'default',
            'connection' => isset($event->batch->options['connection']) ? $event->batch->options['connection'] : 'default',
