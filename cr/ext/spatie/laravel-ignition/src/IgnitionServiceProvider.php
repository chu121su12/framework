...
class IgnitionServiceProvider extends ServiceProvider
{
    ...
    protected function registerIgnition()/*: void*/
    {
        $this->app->singleton(
            ConfigManager::class,
            function () { return new FileConfigManager(config('ignition.settings_file_path', '')); }
        );
        ...
        $this->app->singleton(IgnitionConfig::class, function () use ($ignitionConfig) { return $ignitionConfig; });

        $this->app->singleton(SolutionProviderRepositoryContract::class, function () use ($solutionProviderRepository) { return $solutionProviderRepository; });

        $this->app->singleton(
            Ignition::class,
            function () { return (new Ignition())
                ->applicationPath(base_path()); }
        );
    }
    ...
    {
        foreach (isset($this->app->config['ignition.recorders']) ? $this->app->config['ignition.recorders'] : [] as $recorder) {
