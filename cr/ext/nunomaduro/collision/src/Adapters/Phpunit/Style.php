...
final class Style
{
    ...
    public function writeErrorsSummary(State $state)/*: void*/
    {
        ...
        $errors = array_values(array_filter($state->suiteTests, function (TestResult $testResult) use ($failTypes) { return in_array(
            $testResult->type,
            $failTypes,
            true
        ); }));
        ...
        array_map(function (TestResult $testResult)/*: void*/ {
            if (! backport_instanceof_throwable($testResult->throwable)) {
                throw new ShouldNotHappen();
            }
...
HTML;
            render(sprintf($renderBody2, $truncateClasses, $testResult->color === 'yellow' ? 'yellow-400' : $testResult->color, $testResult->color === 'yellow' ? 'text-black' : '', $testResult->type, $testCaseName, $description, $throwableClassName));
...
HTML;
            render(sprintf($renderBody1, $testResult->testCaseName, $testResult->description, $color, $seconds));
        }

        $timeElapsedInSlowTests = array_sum(array_map(function (TestResult $testResult) { return $testResult->duration / 1000; }, $slowTests));
...
HTML;
        render(sprintf($renderBody2, $percentageInSlowTestsAsString, $timeElapsedAsString, $timeElapsedInSlowTestsAsString));
    }
    ...
    private function titleLineFrom(/*string */$fg, /*string */$bg, /*string */$title, /*string */$testCaseName, /*int */$todos)/*: string*/
    {
        ...
        $todos = backport_type_check('int', $todos);
...
HTML;
        render(sprintf($renderBody0, $seconds === '' ? '' : 'flex space-x-1 justify-between', $truncateClasses, $result->color, $result->icon, $description, $warning, $seconds));
    }
