...
class QueueFake extends QueueManager implements Fake, Queue
{
    ...
    protected function assertPushedWithChainOfClasses($job, $expectedChain, $callback)
    {
        $matching = $this->pushed($job, $callback)->map->chained->map(function ($chain) {
            return collect($chain)->map(function ($job) {
                return get_class(backport_unserialize($job));
            });
    ...
    public function pushed($job, $callback = null)
    {
        ...
        return collect($this->jobs[$job])->filter(
            function ($data) use ($callback) { return $callback($data['job'], $data['queue'], $data['data']); }
        )->pluck('job');
    }
    ...
    public function shouldFakeJob($job)
    {
        ...
        return $this->jobsToFake->contains(function ($jobToFake) use ($job) {
            $jobToFakeClassName = (string) $jobToFake;
            return $job instanceof $jobToFakeClassName || $job === $jobToFakeClassName;
        });
    }
    ...
    protected function shouldDispatchJob($job)
    {
        ...
        return $this->jobsToBeQueued->contains(
            function ($jobToQueue) use ($job) {
                $class = (string) $jobToQueue;
