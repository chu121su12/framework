...
class MailManager implements FactoryContract
{
    ...
    public function createSymfonyTransport(array $config)
    {
        ...
        $transport = isset($config['transport']) ? $config['transport'] : $this->app['config']['mail.driver'];

        if (isset($this->customCreators[$transport])) {
            return call_user_func($this->customCreators[$transport], $config);
        }

        if (trim(isset($transport) ? $transport : '') === '' ||
    ...
    protected function createResendTransport(array $config)
    {
        return new ResendTransport(
            Resend::client(isset($config['key']) ? $config['key'] : $this->app['config']->get('services.resend.key'))
        );
    }
    ...
    protected function createMailgunTransport(array $config)
    {
        ...
        return $factory->create(new Dsn(
            'mailgun+'.(isset($config['scheme']) ? $config['scheme'] : 'https'),
            isset($config['endpoint']) ? $config['endpoint'] : 'default',
            $config['secret'],
            $config['domain']
        ));
    }
    ...
    {
        if ($options = (isset($config['client']) ? $config['client'] : false)) {
