...
class BlueprintState
{
    ...
    public function __construct(Blueprint $blueprint, Connection $connection, Grammar $grammar)
    {
        ...
        $this->columns = collect($schema->getColumns($table))->map(function ($column) { return new ColumnDefinition([
            'name' => $column['name'],
        ...
        $this->foreignKeys = collect($schema->getForeignKeys($table))->map(function ($foreignKey) { return new ForeignKeyDefinition([
        ...
        switch ($command->name) {
            ...
            case 'dropColumn':
                $this->columns = array_values(
                    array_filter($this->columns, function ($column) use ($command) {
                        return ! in_array($column->name, $command->columns);
                    })
                );
            ...
            case 'dropUnique':
                $this->indexes = array_values(
                    array_filter($this->indexes, function ($index) use ($command) {
                        return $index->index !== $command->index;
                    })
                );
            ...
            case 'dropForeign':
                $this->foreignKeys = array_values(
                    array_filter($this->foreignKeys, function ($fk) use ($command) {
                        return $fk->columns !== $command->columns;
                    })
                );
