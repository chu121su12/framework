...
class Migrator
{
    ...
    protected function runUp($file, $batch, $pretend)
    {
        ...
        $this->write(Task::class, $name, function () use ($migration) {
            return $this->runMigration($migration, 'up');
        });
    ...
    protected function getMigrationsForRollback(array $options)
    {
        ...
        if (($batch = isset($options['batch']) ? $options['batch'] : 0) > 0) {
    ...
    protected function runDown($file, $migration, $pretend)
    {
        ...
        $this->write(Task::class, $name, function () use ($instance) {
            return $this->runMigration($instance, 'down');
        });
    ...
    protected function pretendToRun($migration, $method)
    {
        ...
        $this->write(
            BulletList::class,
            collect($this->getQueries($migration, $method))->map(function ($query) {
                return $query['query'];
            })
        );
    }
    ...
    public function usingConnection($name, callable $callback)
    {
        ...
        return tap($callback(), function () use ($previousConnection) {
            return $this->setConnection($previousConnection);
        });
    }
