...
class Builder implements BuilderContract
{
    ...
    public function createOrFirst(array $attributes = [], array $values = [])
    {
        try {
            ...
            $first = $this->useWritePdo()->where($attributes)->first();
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = false)
    {
        ...
        return collect($orders)
            ->filter(function ($order) { return Arr::has($order, 'direction'); })
            ->values();
    }
    ...
    protected function callScope(callable $scope, array $parameters = [])
    {
        ...
        $scoped = $scope(...array_values($parameters));
        $result = isset($scoped) ? $scoped : $this;
    ...
    protected function groupWhereSliceForScope(QueryBuilder $query, $whereSlice)
    {
        ...
        if ($whereBooleans->contains(function ($logicalOperator) {
            return str_contains($logicalOperator, 'or');
        })) {
    ...
    public static function __callStatic($method, $parameters)
    {
        ...
        if ($method === 'mixin') {
            return static::registerMixin($parameters[0], isset($parameters[1]) ? $parameters[1] : true);
        }
