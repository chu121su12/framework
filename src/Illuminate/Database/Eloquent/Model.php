...
abstract class Model implements Arrayable, ArrayAccess, CanBeEscapedWhenCastToString, HasBroadcastChannel, Jsonable, JsonSerializable, QueueableEntity, Stringable, UrlRoutable
{
    ...
    public function loadMorphAggregate($relation, $relations, $column, $function = null)
    {
        ...
        $this->{$relation}->loadAggregate(isset($relations[$className]) ? $relations[$className] : [], $column, $function);
    ...
    protected function incrementOrDecrement($column, $amount, $extra, $method)
    {
        ...
        if ($this->isClassDeviable($column)) {
            $amount = with(clone $this)->setAttribute($column, $amount)->getAttributeFromArray($column);
        }
    ...
    {
        try {
            $json = backport_json_encode($this->jsonSerialize(), $options, 512, true);
    ...
    {
        $defaults = array_values(array_filter(\array_merge([
            $this->getKeyName(),
    ...
    {
        return $query->where(isset($field) ? $field : $this->getRouteKeyName(), $value);
    }
